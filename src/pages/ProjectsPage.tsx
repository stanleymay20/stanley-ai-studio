...
const ProjectsPage = () => {
  ...
  const usingFallback = !loading && projects.length === 0;
  const featuredCount = visibleProjects.filter((project) => project.featured).length;

  return (
    <div className="min-h-screen bg-background">
      <Seo
        title="AI & Data Science Projects | Stanley Osei-Wusu"
        description="Selected AI engineering, decision-intelligence, data-engineering, and Bayesian data-science projects by Stanley Osei-Wusu."
        path="/projects"
      />
      <Header />
      ...
      <main className="pt-24 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-10">
            <h1 className="text-3xl font-bold text-foreground mb-2">AI & Data Science Projects</h1>
            <p className="text-muted-foreground">
              Selected work in applied AI, decision intelligence, data engineering, and probabilistic modeling.
            </p>
            {!loading && (
              <p className="text-sm text-muted-foreground mt-2">
                Showing {visibleProjects.length} project{visibleProjects.length !== 1 ? "s" : ""}
                {featuredCount > 0 ? ` · ${featuredCount} featured` : ""}
                {usingFallback ? " · verified public GitHub repositories" : ""}
              </p>
            )}
          </div>
